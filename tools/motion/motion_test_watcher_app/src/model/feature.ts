...
function createSubFeatures(feature: Feature): Feature[] {
  ...
  switch (type) {
    ...
    case 'animatedVisibilityTransitions':
      return [
        ...new Set(
          feature.dataPoints.flatMap((it) => Object.keys(it as DataPointObject))
        ),
      ]
        .sort()
        .map((it) =>
      ...
      return [
        ...new Set(
          feature.dataPoints.flatMap((it) =>
            it ? Object.keys(it as DataPointObject) : []
          )
        ),
      ]
        .sort()
        .flatMap((name) => {
          ...
          return [
            createSubFeature(feature, name, type, (point) => point[name]),
          ];
        });
  }
