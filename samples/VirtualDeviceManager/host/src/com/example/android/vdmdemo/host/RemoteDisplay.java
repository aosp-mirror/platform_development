  ...
  public RemoteDisplay(
    ...
    virtualDisplay =
        virtualDevice.createVirtualDisplay(
            new VirtualDisplayConfig.Builder("VirtualDisplay" + remoteDisplayId, width, height, dpi)
                .setFlags(flags)
        ...
        (result) -> {
          switch (result) {
            ...
            case VirtualDeviceManager.LAUNCH_FAILURE_PENDING_INTENT_CANCELED:
              Log.w(TAG, "launchAppOnDisplay: Launching app failed with reason: " + result);
              break;
            default:
              Log.w(TAG, "launchAppOnDisplay: Unexpected result when launching app: " + result);
          }
        });
  }
    ...
    if (rotation != virtualDisplay.getDisplay().getRotation()) {
      ...
      remoteIo.sendMessage(
          RemoteEvent.newBuilder()
              ...
              .setDisplayRotation(DisplayRotation.newBuilder().setRotationDegrees(rotationDegrees))
              .build());
    }
  }
  ...
  private void processRemoteEvent(RemoteEvent event) {
    ...
    if (event.hasHomeEvent() && supportsHome) {
      ...
      context.startActivity(
          homeIntent, ActivityOptions.makeBasic().setLaunchDisplayId(getDisplayId()).toBundle());
        ...
        if (navigationTouchpad == null) {
          ...
          navigationTouchpad =
              virtualDevice.createVirtualNavigationTouchpad(
                  new VirtualNavigationTouchpadConfig.Builder(displaySize.x, displaySize.y)
                      .setAssociatedDisplayId(getDisplayId())
                      .setInputDeviceName("vdmdemo-navtouchpad" + remoteDisplayId)
                      .build());
        }
          ...
          keyboard =
              virtualDevice.createVirtualKeyboard(
                  new VirtualKeyboardConfig.Builder()
                      .setInputDeviceName("vdmdemo-keyboard" + remoteDisplayId)
                      .setAssociatedDisplayId(getDisplayId())
                      .build());
        }
    ...
    if (inputEvent.hasMouseButtonEvent()) {
      ...
      mouse.sendRelativeEvent(
          new VirtualMouseRelativeEvent.Builder()
              .setRelativeX(inputEvent.getMouseRelativeEvent().getX() - cursorPosition.x)
              .setRelativeY(inputEvent.getMouseRelativeEvent().getY() - cursorPosition.y)
              .build());
    } else {
